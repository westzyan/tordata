...
public interface OnionPerfBuildTimesDao {
    ...
    @Select("select * from onionperf_buildtimes where perf_date >= #{start} and perf_date <= #{end}")
    @Results({
            @Result(property = "date", column = "perf_date")
    })
    public List<OnionPerfBuildTimes> listOnionPerfLatenciesByCondition(@Param("start") String start, @Param("end") String end);
